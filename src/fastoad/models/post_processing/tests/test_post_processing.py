...
from ..wing_lift_distibution import WingLiftDistribution  # TestXfoil
...
def test_wing_lift_distribution():
    mach = 0.5
    reynolds = 100000
    thickness_ratio = 0.12

    ivc = om.IndepVarComp()
    ivc.add_output("xfoil:mach", val=mach)
    ivc.add_output("xfoil:reynolds", val=reynolds)
    ivc.add_output("data:geometry:wing:thickness_ratio", val=thickness_ratio)

    # problem = run_system(
    #     TestXfoil(),ivc
    # )

    problem = run_system(WingLiftDistribution(), ivc)
